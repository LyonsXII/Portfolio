...
import { StyledIntroContentContainer, StyledContentFlexbox, StyledTitle, StyledMinorTitle, StyledBodyText, StyledImage, StyledTitleContainer, StyledGridOverlay, StyledGridCell, StyledIntroButtonContainer, StyledSpacer, StyledButtonsContainer, StyledIntroContainer, StyledSVG } from "./Introduction.styles";
...
function IntroText({ id, title, textA, textB, layout, current, tempCurrent, changeSection, $expandIntroText, imgA }) {
  ...
  useEffect(() => {
    ...
    const flickerRandomCells = () => {
      if (isCancelled) return;
  
      // Pick how many cells to flicker (e.g., 1–4)
      const count = Math.floor(Math.random() * 4) + 1;
  
      // Generate `count` unique indexes
      const indexes = new Set();
      while (indexes.size < count) {
        indexes.add(Math.floor(Math.random() * 800)); // 100 = total number of cells
      }
      ...
      setFlickerMap((prev) => {
        const updated = { ...prev };
        indexArray.forEach((i) => (updated[i] = true));
        return updated;
      });
  
      // Clear them after a flicker duration
      setTimeout(() => {
        setFlickerMap((prev) => {
          const updated = { ...prev };
          indexArray.forEach((i) => (updated[i] = false));
          return updated;
        });
      }, 200); // flicker duration
  
      // Call this function again after a random delay
      const nextDelay = Math.random() * 1000 + 200; // between 200ms and 1200ms
      setTimeout(flickerRandomCells, nextDelay);
    };
  
    flickerRandomCells(); // start the loop
      ...
      <StyledIntroContainer {...containerProps} $expandIntroText={$expandIntroText} $current={current}>
        <StyledTitleContainer theme={theme}>
          ...
          {[...Array(400)].map((_, index) => (
            <StyledGridCell key={index} $flicker={!!flickerMap[index]} />
          ))}
          </StyledGridOverlay>
